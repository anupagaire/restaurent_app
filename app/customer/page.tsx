...
import { ShoppingBag, User, ChevronRight, Truck, Clock, Star, MapPin, UtensilsCrossed } from 'lucide-react';
    ...
    const load = async () => {
      try {
        const [meRes, ordersRes] = await Promise.all([
          ...
          apiFetch('/api/v1/orders/?page_size=50'),
        ]);
        ...
        const list: Order[] = Array.isArray(orders)
          ? orders
          : (orders.results ?? []);
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <button key={href} onClick={() => router.push(href)}
            ...
            <ChevronRight size={13} className="mt-2 transition-transform group-hover:translate-x-1"
              style={{ color: '#b8936a' }} />
          </button>
        ))}
      </div>
      ...
      {restaurants.length > 0 && (
        <div>
          <h2 className="font-bold text-base mb-3" style={{ color: '#1e0f02', fontFamily: 'Georgia, serif' }}>
            🍽️ Restaurants You've Visited
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {restaurants.map(r => (
              <div key={r.name}
                className="bg-white rounded-2xl px-4 py-4 border border-gray-100 flex items-center gap-4">
                ...
                <div className="text-right shrink-0">
                  <p className="text-xs text-gray-400">{formatTime(r.lastOrder)}</p>
                </div>
              </div>
            ))}
          </div>
