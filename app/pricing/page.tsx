...
function PromoInput({
  ...
  const handleApply = async () => {
    ...
    // Simulate API call
    setTimeout(() => {
      if (trimmed === 'SAVE20' || trimmed === 'WELCOME') {
        setStatus('valid');
        setMessage('Rs. 500 discount applied!');
        onApply({
          valid: true,
          discount_amount: '500',
          final_amount: '1499',
...
function PlanCard({
  ...
  const accent = planAccent(plan);
  const price = parseFloat(plan.price);
  const finalPrice = promoResult ? parseFloat(promoResult.final_amount || plan.price) : price;
  const hasDiscount = promoResult && finalPrice < price;
  const isFree = price === 0 || plan.is_trial;
