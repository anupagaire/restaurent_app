...
export default function RestaurantServicesAdminPage() {
  ...
  const handleImageUpload = async (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!data.id) {
      alert('⚠️ Please save the services page FIRST before uploading photos!');
      return;
    }
  ...
  return (
    <div className="min-h-screen ">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-primary">Services Page Editor</h1>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-primary">Service Photos</h2>
            <button
              onClick={addPhoto}
              disabled={!data.id}
          ...
          {!data.id && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm text-yellow-800">
              ⚠️ Please save the services page first before adding photos.
            </div>
          )}
